...
public class LoginAdmin {
    ...
    public void seConnecter() {
        ...
        loggedInUser = findUserByEmailAndPassword2(email, motDePasse);
        ...
        if (loggedInUser != null) {
            ...
            try {
                // Create an instance of LoginAdmin
                LoginAdmin loginAdminController = new LoginAdmin();

                // Set loggedInUser in LoginAdmin controller
                loginAdminController.setLoggedInUser(loggedInUser);
                System.out.println(loggedInUser);

                // Define the path to the FXML file based on the role
                String fxmlFile = "";
                switch (role) {
                    case "Admin":
    ...
    private User findUserByEmailAndPassword2(String email, String password) {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
             PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM user WHERE email = ? AND mdp = ?")) {
